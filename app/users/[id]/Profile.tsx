...
export default function Profile({ author }: { author: { id: number; name: string; email: string } }) {
    return (
        <div className="flex flex-col items-center gap-2 text-center">
            <div className={`flex size-20 items-center justify-center rounded-full bg-gradient-to-br ${avatarGradient(author.id)} text-3xl text-white shadow-lg`}>
                {author.name.charAt(0).toUpperCase()}
            </div>
            <h2 className="pt-2 text-2xl">{author.name}</h2>
